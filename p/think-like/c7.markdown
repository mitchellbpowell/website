...
{% highlight ruby %}
for x in range(1, 10):
    print("{} takes {} steps to resolve.".format(x, collatz\_steps(x)))
{% endhighlight %}
...
for x in range(1, 100):
    if collatz\_steps(x) > 100:
        print("{} takes {} steps to resolve.".format(x, collatz\_steps(x)))
        print(print\_collatz(x))
        break
{% endhighlight %}
...
for x in range(1, 10\*\*6):
    this\_result = collatz\_steps(x)
    if this\_result > high:
        high = this\_result
...
assert odd\_nums\_in\_list(\[\]) == 0
assert odd\_nums\_in\_list(\[1\]) == 1
assert odd\_nums\_in\_list(\[-1\]) == 1
assert odd\_nums\_in\_list(\[0\]) == 0
assert odd\_nums\_in\_list(\[1, 7, -3\]) == 3
assert odd\_nums\_in\_list(\[1, 2, 3, 4, 5\]) == 3
{% endhighlight %}
...
assert sum\_even\_nums(\[\]) == 0
assert sum\_even\_nums(\[2\]) == 2
assert sum\_even\_nums(\[9\]) == 0
assert sum\_even\_nums(\[1, 2, 3, 4\]) == 6
assert sum\_even\_nums(\[1, 2, 3, 7, -4, 12\]) == 10
{% endhighlight %}
...
assert sum\_neg\_nums(\[\]) == 0
assert sum\_neg\_nums(\[1\]) == 0
assert sum\_neg\_nums(\[-1, -2, -3\]) == -6
assert sum\_neg\_nums(\[1, 7, -6, -14, 12, 87\]) ==  -20
{% endhighlight %}
...
assert len\_5\_words(\[\]) == 0
assert len\_5\_words(\["Jim"\]) == 0
assert len\_5\_words(\["Jimmy"\]) == 1
assert len\_5\_words(\["The", "quick", "brown", "fox", 
...
assert up\_to\_even(\[\]) == 0
assert up\_to\_even(\[2\]) == 0
assert up\_to\_even(\[7, 3, 4\]) == 10
assert up\_to\_even(\[6, 5, 5, 5, 5\]) == 0
assert up\_to\_even(\[-4, 1, 3, 5\]) == 0
assert up\_to\_even(\[-3, 1, 6\]) == -2
assert up\_to\_even(\[1.5, 3, 4\]) == 4.5
assert up\_to\_even(\[1, 3, 5, 7\]) == 16
{% endhighlight %}
...
assert words\_before\_sam(\[\]) == 0
assert words\_before\_sam(\["sam"\]) == 0
assert words\_before\_sam(\["i", "am", "sam"\]) == 2
assert words\_before\_sam(\["sam", "i", "am"\]) == 0
assert words\_before\_sam(\["to", "be", "or", "not", "to", "be"\]) == 6
{% endhighlight %}
...
```
{% highlight ruby %}
def print\_triangular\_numbers(n):
    counter = 0
...
{% highlight ruby %}
test(is\_prime(11))
test(not is\_prime(35))
test(is\_prime(19911121))
{% endhighlight %}
...
We can also answer the question about the 100 students, which I've done elsewhere and hope to create a link to here as I migrate site materials over.[^1]
...
turns\_and\_steps = \[(160, 20), (-43, 10), (270, 8), (-43, 12)\]

# Simulate the actual stumbling around.
for x in turns\_and\_steps:
    # The pirate turns.
    pirate.left(x\[0\])
...
def travel(ttl, turns\_and\_steps):
    """Takes a turtle ttl and a specially formatted list of turns\_and\_steps,
...
def travel(ttl, turns\_and\_steps):
    """Takes a turtle ttl and walks it through a series of turns\_and\_steps."""
    for x in turns\_and\_steps:
        ttl.left(x\[0\])
        ttl.forward(x\[1\])
...
regular\_line = 60
little\_diagonal = regular\_line / 2\*\*0.5
big\_diagonal = little\_diagonal \* 2

# First drawing.
turns\_and\_steps = \[(0, regular\_line), 
                   (135, little\_diagonal),
                   (90, little\_diagonal),
                   (45, regular\_line),
                   (90, regular\_line),
                   (90, regular\_line)
...
tess.penup()                       
travel(tess, \[(-90, regular\_line), 
              (-45, big\_diagonal), 
              (45, 0)\])

# Second drawing.
tess.pendown()       
travel(tess, \[(180, regular\_line),
              (-90, regular\_line),
              (-135, big\_diagonal),
              (135, regular\_line),
              (90, regular\_line),
...
tess.penup()                           
travel(tess, \[(0, big\_diagonal), (45, 0)\])
...
tess.penup()                        
travel(tess, \[(135, regular\_line \* 1.5),
              (45, big\_diagonal)
\])

# Fourth drawing.
tess.pendown()        
travel(tess, \[(135, regular\_line),
              (90, regular\_line),
              (90, regular\_line),
              (90, regular\_line),
              (135, big\_diagonal),
...
{% highlight ruby %}
test(num\_digits(0) == 1)
test(num\_digits(-12345) == 5)
{% endhighlight %}
...
assert num\_digits(-100) == 3
assert num\_digits(-99) == 2
assert num\_digits(-11) == 2
assert num\_digits(-10) == 2
assert num\_digits(-9) == 1
assert num\_digits(-1) == 1
assert num\_digits(0) == 1 
assert num\_digits(1) == 1
assert num\_digits(2) == 1
assert num\_digits(9) == 1
assert num\_digits(10) == 2
assert num\_digits(11) == 2
assert num\_digits(99) == 2
assert num\_digits(100) == 3
assert num\_digits(101) == 3 
assert num\_digits(-12345) == 5
{% endhighlight %}
...
{% highlight ruby %}
test(num\_even\_digits(123456) == 3)
test(num\_even\_digits(2468) == 4)
test(num\_even\_digits(1357) == 0)
test(num\_even\_digits(0) == 1)
{% endhighlight %}

These tests pass with the following code:

{% highlight ruby %}
def num\_even\_digits(n):
    n = abs(n)              # Strip off any potential negative symbol.
    n = str(n)              
    list\_of\_even\_digits = \["0", "2", "4", "6", "8"\]

    counter = 0
    for char in n:
        if char in list\_of\_even\_digits:
            counter += 1
    return counter

assert num\_even\_digits(123456) == 3
assert num\_even\_digits(2468) == 4
assert num\_even\_digits(1357) == 0
assert num\_even\_digits(0) == 1
{% endhighlight %}
...
{% highlight ruby %}
def sum\_of\_squares(xs):
    sum = 0
    for x in xs:
        sum += x\*\*2
    return sum

assert sum\_of\_squares(\[2, 3, 4\]) == 29
assert sum\_of\_squares(\[\]) == 0
assert sum\_of\_squares(\[2, -3, 4\]) == 29
{% endhighlight %}

[^1]: Every so often, I run a check of all my site pages for any broken links. In order to remind myself that migrating my "100 kids" problem is a to-do, and that I need to link out to it from here, here's an intentionally broken link: [link to nowhere](link-to-nowhere.html)
